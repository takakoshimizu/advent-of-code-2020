defmodule Advent2020Day3Test do
  ...
  test "next_spot returns new position by default movement strat" do
    {_tree, {x, y}} = Advent2020.Day3.next_spot(@input, {0, 0})
    assert x === 3
    assert y === 1
  end

  test "next_spot returns new position by custom movement strategy" do
    {_tree, {x, y}} = Advent2020.Day3.next_spot(@input, {0, 0}, {4, 4})
    assert x === 4
    assert y === 4
  end

  test "next_spot should wrap around x coords" do
    {_tree, {x, _y}} = Advent2020.Day3.next_spot(@input, {9, 0})
    assert x === 1
  end

  test "next_spot returns whether next spot is tree" do
  ...
  test "traverse_map returns state when y is at end of map" do
    trees = Advent2020.Day3.traverse_map(@input, nil, {5, {0, 11}})
    assert trees === 5
  end

  test "traverse_map traverses until reaching end of map; returns number of trees encountered" do
    trees = Advent2020.Day3.traverse_map(@input)
    assert trees === 7
  end

  test "traverse_map traverses with custom movement strategy" do
